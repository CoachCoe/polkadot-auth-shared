...
export class WalletProviderService {
  ...
  private detectAvailableWallets(): void {
    ...
    this.availableWallets = walletConfigs.map((config) => ({
      ...config,
      isAvailable: !!(
        window.injectedWeb3 && window.injectedWeb3[config.extensionName]
      ),
    }));
  }
  ...
  async connectWallet(walletName: string): Promise<WalletConnection> {
    ...
    const walletConfig = this.availableWallets.find(
      (w) => w.name === walletName,
    );
    if (!walletConfig) {
      throw ErrorService.createError(
    ...
    try {
      ...
      const extension =
        await window.injectedWeb3[walletConfig.extensionName].enable(
          "Polkadot SSO",
        );
