import { ExchangeRateConfig } from "../services/exchangeRateService.js";
...
export class ProductionConfigManager {
  ...
  private loadSecurityConfig(): SecurityConfig {
    ...
    return {
      ...
      allowedOrigins: this.parseArrayEnv("CORS_ALLOWED_ORIGINS", [
        "http://localhost:3000",
      ]),
  ...
  private loadExchangeRateConfig(): ExchangeRateConfig {
    return {
      provider:
        (process.env.EXCHANGE_RATE_PROVIDER as "coingecko" | "mock") ||
        "coingecko",
      ...
      cacheTimeout: parseInt(
        process.env.EXCHANGE_RATE_CACHE_TIMEOUT || "60000",
      ),
    };
  }
  ...
  private loadRemittanceConfig(): RemittanceConfig {
    return {
      ...
      supportedCurrencies: this.parseArrayEnv(
        "REMITANCE_SUPPORTED_CURRENCIES",
        ["USD", "ARS", "BRL"],
      ),
      treasuryAddress: process.env.REMITANCE_TREASURY_ADDRESS || "",
      minDeposit: parseFloat(process.env.REMITANCE_MIN_DEPOSIT || "10"),
      maxWithdrawal: parseFloat(
        process.env.REMITANCE_MAX_WITHDRAWAL || "10000",
      ),
  ...
  private validateConfig(): void {
    ...
    if (this.config.database.type !== "sqlite" && !this.config.database.host) {
      this.validationErrors.push(
        "DATABASE_HOST is required for non-SQLite databases",
      );
    }

    // Validate remittance config
    if (
      this.config.remittance.enabled &&
      !this.config.remittance.treasuryAddress
    ) {
      this.validationErrors.push(
  ...
  getConfigSummary(): Record<string, any> {
    ...
    return {
      ...
      security: {
        enableSignatureVerification:
          this.config.security.enableSignatureVerification,
